...
<head>
    ...
    <?php $layoutClass->MontarHead('Tipos de clipping') ?>
</head>
    ...
    <div class="d-flex" id="wrapper">
        ...
        <div id="page-content-wrapper">
            ...
            <div class="container-fluid p-2">
                ...
                <?php $layoutClass->cardDescription('<i class="fa-solid fa-newspaper"></i> Adicionar tipos de clipping', '<p class="card-text mb-0">Seção para gerenciamento de tipos de clipping</p>') ?>
                <div class="row">
                    <div class="col-12">
                        <div class="card shadow-sm mb-2">
                            <div class="card-body p-2">
                                ...
                                if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_salvar'])) {
                                    $clippingTipo = [
                                        'clipping_tipo_nome' => $_POST['nome'],
                                        'clipping_tipo_descricao' => $_POST['descricao'],
                                    ];

                                    $resultado = $clippingTipoController->NovoClippingTipo($clippingTipo);
                ...
                <?php
                $tipos = $clippingTipoController->ListarClippingTipos();
                $tabela = [];

                if ($tipos['status'] == 'success' && $tipos['status'] != 'empty') {
                    foreach ($tipos['dados'] as $tipo) {
                        $tabela[] = [
                            'Nome' => $tipo['clipping_tipo_nome'],
                            'Descrição' => $tipo['clipping_tipo_descricao'],
                            'Criado em | por' => date('d/m', strtotime($tipo['clipping_tipo_criado_em'])) . ' | ' . $tipo['usuario_nome'],
                        ];
                    }
