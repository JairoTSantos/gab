...
class ComissoesController {
            ...
            $pauta[] = array_merge(
                ...
                !empty($proposicao['proposicaoRelacionada_']) ? [
                    'proposicao' => [
                        ...
                        'apensados_do_deputado' => $this->BuscarApensados($proposicao['proposicaoRelacionada_']['id'])
                    ]
                ] : []
            );
        }
    ...
    public function BuscarAutores($id) {
        ...
        foreach ($dadosAutores['dados'] as $autor) {
            $autores[] = [
                'autor_id' => $autor['uri'],
                'autor_flag' => $autor['nome'] === $config['deputado']['nome_deputado'] ? true : false,
    ...
    public function BuscarApensados($id) {
        $dadosApensados = getJson('https://dadosabertos.camara.leg.br/api/v2/proposicoes/' . $id . '/relacionadas');
        $apensados = [];

        foreach ($dadosApensados['dados'] as $apensado) {
            if ($apensado['siglaTipo'] === 'PL') {
                $autores = $this->BuscarAutores($apensado['id']);

                $temAutorFlagTrue = false;
                foreach ($autores as $autor) {
                    if ($autor['autor_flag'] === true) {
                        $temAutorFlagTrue = true;
                        break;
                    }
                }
                ...
                if ($temAutorFlagTrue) {
                    $apensados[] = [
                        'apensado_id' => $apensado['id'],
                        'apensado_titulo' => $apensado['siglaTipo'] . ' ' . $apensado['numero'] . '/' . $apensado['ano'],
                        'apensado_ementa' => $apensado['ementa']
                    ];
                }
            }
        }
